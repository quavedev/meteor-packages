...
if (Meteor.isClient) {
  ...
  Tinytest.addAsync(
    'insert - only one of two collection documents should be allowed to be inserted, and should carry the extra server and client properties',
    function (test, next) {
      collection.before.insert(function (userId, doc) {
      ...
      InsecureLogin.ready(function () {
        Meteor.call(
          'test_insert_allow_reset_collection',
          function (nil, result) {
            collection.insert(
              { start_value: true, allowed: false },
              function (err1, id1) {
                collection.insert(
                  { start_value: true, allowed: true },
                  function (err2, id2) {
                    test.equal(
                      collection
                        .find({

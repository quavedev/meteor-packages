...
const collection1 = new Mongo.Collection('test_insert_mongoid_collection1', {
  idGeneration: 'MONGO',
});
...
Tinytest.addAsync(
  'meteor_1_4_id_object - after insert hooks should be able to cope with object _id with ops property in Meteor 1.4',
  function (test, next) {
    const key = Date.now();
    ...
    const aspect1 = collection.after.insert(function (nil, doc) {
      if (doc && doc.key && doc.key === key) {
        ...
        test.isFalse(
          Object(doc._id) === doc._id,
          '_id property should not be an object'
        );
      }
    });
...
Tinytest.addAsync(
  'meteor_1_4_id_object - after insert hooks should be able to cope with Mongo.ObjectID _id with _str property in Meteor 1.4',
  function (test, next) {
    const key = Date.now();

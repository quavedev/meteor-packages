...
Tinytest.addAsync(
  'findone - selector should be {} when called without arguments',
  function (test, next) {
...
Tinytest.addAsync(
  'findone - selector should have extra property',
  function (test, next) {
    const collection = new Mongo.Collection(null);
    ...
    InsecureLogin.ready(function () {
      collection.insert(
        { start_value: true, before_findone: true },
        function (err, id) {
          if (err) throw err;
          test.notEqual(
            collection.findOne(
              { start_value: true, bogus_value: true },
              { test: 1 }
            ),
            undefined
          );
...
Tinytest.addAsync(
  'findone - tmp variable should have property added after the find',
  function (test, next) {
    const collection = new Mongo.Collection(null);
    const tmp = {};

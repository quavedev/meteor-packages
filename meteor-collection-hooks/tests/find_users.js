...
Tinytest.addAsync(
  'users - find hooks should be capable of being used on special Meteor.users collection',
  function (test, next) {
    // eslint-disable-next-line array-callback-return
    const aspect1 = Meteor.users.before.find(
    ...
    // eslint-disable-next-line array-callback-return
    const aspect2 = Meteor.users.after.find(
      function (userId, selector, options) {
        if (selector && selector.test) {
          selector.b = 1;
        }
...
Tinytest.addAsync(
  'users - find hooks should be capable of being used on wrapped Meteor.users collection',
  function (test, next) {
    ...
    Meteor.users.find = function (selector = {}, options = {}) {
      return MeteorUsersFind.call(this, selector, {

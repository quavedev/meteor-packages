...
if (Meteor.isServer) {
  Tinytest.addAsync(
    'update - collection1 document should have extra property added to it before it is updated',
    function (test, next) {
      const tmp = {};
...
if (Meteor.isClient) {
  ...
  Tinytest.addAsync(
    'update - collection2 document should have client-added and server-added extra properties added to it before it is updated',
    function (test, next) {
      let c = 0;
      const n = () => {
        if (++c === 2) {

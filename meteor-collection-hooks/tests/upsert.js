...
Tinytest.addAsync(
  ...
  function (test, next) {
    ...
    collection.after.upsert(function () {
      counts.after.upsert++;
    });
      ...
      collection.remove({ test: true }, function (err) {
        ...
        collection.upsert(
          ...
          function (err, obj) {
            ...
            collection.upsert(
              obj.insertedId,
              { test: true, step: 'update' },
              function (err) {
                if (err) throw err;
                test.equal(
                  counts.before.insert,
                  0,
                  'before.insert should be 0'
                );
                test.equal(
                  counts.before.update,
                  0,
                ...
                test.equal(
                  counts.before.remove,
                  0,
                  'before.remove should be 0'
                );
                test.equal(
                  counts.before.upsert,
                  2,
                  'before.insert should be 2'
                );
...
Tinytest.addAsync(
  'upsert before.upsert can stop the execution',
  function (test, next) {
    const collection = new Mongo.Collection(null);
...
if (Meteor.isServer) {
  Tinytest.add(
    'issue #156 - upsert after.insert should have a correct doc using $set in synchronous environment',
    function (test) {
      const collection = new Mongo.Collection(null);
      ...
      collection.after.insert(function (userId, doc) {
        ...
        test.equal(
          doc.step,
          'insert-sync',
          'doc should have a step property equal to insert-sync'
        );
      });
      ...
      collection.upsert(
        { test: true },
        { $set: { test: true, step: 'insert-sync' } }
      );
    }
  );
}

...
Tinytest.add(
  'compat - "new Mongo.Collection" should not throw an exception',
  function (test) {
    try {
...
Tinytest.addAsync(
  'compat - hooks should work for "new Mongo.Collection"',
  function (test, next) {
        ...
        function (err) {
          ...
          collection.remove({ _id: id }, function (nil) {
            test.equal(
              counts.before.insert,
              1,
              'before insert should have 1 count'
            );
            test.equal(
              counts.before.update,
              1,
              'before update should have 1 count'
            );
            test.equal(
              counts.before.remove,
              1,
              'before remove should have 1 count'
            );
            test.equal(
              counts.after.insert,
              1,
              'after insert should have 1 count'
            );
            test.equal(
              counts.after.update,
              1,
              'after update should have 1 count'
            );
            test.equal(
              counts.after.remove,
              1,
              'after remove should have 1 count'
            );
            next();
          });
        }

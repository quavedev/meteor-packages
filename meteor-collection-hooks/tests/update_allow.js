...
if (Meteor.isClient) {
  ...
  Tinytest.addAsync(
    'update - only one of two collection documents should be allowed to be updated, and should carry the extra server and client properties',
    function (test, next) {
      collection.before.update(function (userId, doc, fieldNames, modifier) {
      ...
      InsecureLogin.ready(function () {
        Meteor.call(
          'test_update_allow_reset_collection',
          function (nil, result) {
            function start(id1, id2) {
              collection.update(
                { _id: id1 },
                { $set: { update_value: true, allowed: true } },
                function (err1) {
                  collection.update(
                    { _id: id2 },
                    { $set: { update_value: true, allowed: false } },
                    function (err2) {
                      test.equal(
                        collection
                          .find({

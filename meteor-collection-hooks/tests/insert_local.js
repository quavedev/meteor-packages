...
Tinytest.addAsync(
  'insert - local collection document should have extra property added before being inserted',
  function (test, next) {
      ...
      collection.insert({ start_value: true }, function (err, id) {
        ...
        if (Meteor.isServer) {
          test.equal(
            tmp.typeof_userId,
            'undefined',
            'Local collection on server should NOT know about a userId'
          );
        } else {
          test.equal(
            tmp.typeof_userId,
            'string',
            'There should be a userId on the client'
          );
        }
        test.equal(
          collection
...
Tinytest.addAsync(
  'insert - local collection should fire after-insert hook',
  function (test, next) {
    const collection = new Mongo.Collection(null);

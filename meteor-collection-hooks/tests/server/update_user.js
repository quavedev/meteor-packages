...
Tinytest.addAsync(
  'update - Meteor.users collection document should have extra property added before being updated',
  function (test, next) {
    const collection = Meteor.users;

    function start() {
      const aspect1 = collection.before.update(
      ...
      const aspect2 = collection.after.update(
        function (userId, doc, fieldNames, modifier, options) {
          test.isTrue(
            modifier !== undefined && options !== undefined,
            'modifier and options should not be undefined when fetchPrevious is false issue #97 and #138'
          );
        },
        { fetchPrevious: false }
      );
          ...
          function (err) {
            ...
            test.equal(
              collection
                ...
                .count(),
              1,
              'number of users found should be 1'
            );

  ...
  function (
    ...
    if (!suppressAspects) {
      if (
        ...
        docs = CollectionHooks.getDocs
          .call(this, instance, selector, options)
          .fetch();
    ...
    const afterUpdate = (affected, err) => {
      if (!suppressAspects && !isEmpty(aspectGroup.update.after)) {
        ...
        const docs = CollectionHooks.getDocs
    ...
    const afterInsert = (_id, err) => {
      if (!suppressAspects && !isEmpty(aspectGroup.insert.after)) {
        const doc = CollectionHooks.getDocs
          .call(this, instance, { _id }, selector, {})
          .fetch()[0]; // 3rd argument passes empty object which causes magic logic to imply limit:1
    ...
    if (async) {
      ...
      const ret = CollectionHooks.directOp(() =>

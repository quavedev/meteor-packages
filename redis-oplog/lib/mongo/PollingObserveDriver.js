...
Object.assign(PollingObserveDriver.prototype, {
  // Do nothing, this is just used to have compatibility with the init call when initializing it.
  init() {},
  ...
  _unthrottledEnsurePollIsScheduled: function () {
    ...
    self._taskQueue.queueTask(async function () {
  ...
  _suspendPolling: async function () {
    ...
    await self._taskQueue.runTask(function () {}); // Confirm that there is only one "poll" (the fake one we're pretending to
    // have) scheduled.
  ...
  _pollMongo: async function () {
    ...
    self._testOnlyPollCallback && (await self._testOnlyPollCallback()); // Save the list of pending writes which this round will commit.
